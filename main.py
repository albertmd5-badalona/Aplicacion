...
# --- COMPONENTES UI ---
def Title(text):
    return ft.Text(text, size=24, weight="bold", color=C_PRIMARY)
...
def Card(content):
    return ft.Container(
        ...
        border_radius=12,
        bgcolor=C_SURFACE,
        shadow=ft.BoxShadow(spread_radius=1, blur_radius=10, color="grey", offset=ft.Offset(0, 5))
    )
...
def view_calendar(page):
    ...
    def generate_calendar(e=None):
        ...
        for month in range(1, 13):
            ...
            month_col.controls.append(ft.Text(month_name, weight="bold", color=C_PRIMARY))
            ...
            for week in cal:
                ...
                for day in week:
                    if day == 0:
                        row_controls.append(ft.Container(width=25, height=25))
                    else:
                        ...
                        if d_str in ex_map:
                            ...
                            content = ft.Container(
                             ...
                             content = ft.Text(str(day), size=10, color="grey")
    ...
    return ft.Column([
        ...
        ft.Row([current_emp, ft.Text("Click green to toggle (Work -> Vac -> Hol)", size=12, color="grey")]),
        ft.Divider(),
        ft.Container(content=cal_container, expand=True)
    ], expand=True)

def view_planning(page):
    rows_container = ft.Column(scroll="auto")
    ...
    def load_matrix():
        ...
        for w in weeks:
            header_row.append(ft.Container(width=60, content=ft.Text(w[5:], size=10, weight="bold")))
    ...
    load_matrix()
    return ft.Column([Title("🔢 Capacity Plan"), ft.Container(content=rows_container, expand=True)], expand=True)
...
def main(page: ft.Page):
    ...
    page.appbar = ft.AppBar(
        leading=ft.Icon("grid_view", color=C_PRIMARY),
        leading_width=40,
        title=ft.Text("CorpManager v1.0", weight="bold", color=C_TEXT),
        center_title=False,
        bgcolor="white",
        elevation=4,
        actions=[
            ft.IconButton("people", tooltip="Employees", on_click=lambda e: navigate(0)),
            ft.IconButton("rocket_launch", tooltip="Projects", on_click=lambda e: navigate(1)),
            ft.IconButton("calendar_month", tooltip="Calendar", on_click=lambda e: navigate(2)),
            ft.IconButton("table_chart", tooltip="Planning", on_click=lambda e: navigate(3)),
